...
void LSM6DSOXClass::end()
{
  ...
  _wire->endTransmission(); // Ensure the transmission is complete before ending
  _wire->end();
}

void LSM6DSOXClass::customI2CPins(uint8_t sda, uint8_t scl)
{
  // Set custom SDA and SCL pins
  _wire->updatePins(sda, scl);
}
